...
<table class="table table-bordered">
	...
	<tbody id="table-list">
		<tr>
			...
			<td><a href="" data-eq="{eq}" class="text-normal">{Point}</a></td>
...
function get_list( pagination ){
	...
	var list_url   = '/groupware/rule/lists/';
		...
		success: function(data){
			...
			if(list.length>0){
				for (var i in list){
					...
					template = template.replace(/{point}/gi    ,(operator=='0'?'+':'-') + point);
				...
				$table_list.html('<tr><td colspan="5">등록된 내용이 없습니다.</td></tr>');
				$pagination.html(pagination);
			}
...
function onlick(data){
	...
	$('#in_title').val(title);
	$('#in_operator').val(operator);
	$('#in_point').val((operator=='0'?'+':'-')+point);
	$('#text_title').text(title);
	$('#text_point').text((operator=='0'?'+':'-')+point);
	$('#text_sum').text('준비');
